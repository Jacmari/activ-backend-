...
// ---- NEW: hard timeout wrapper to avoid Heroku H12s ----
...
// ---- UPDATED: plaidPost uses fetchWithTimeout and normalized errors ----
async function plaidPost(path, body) {
  ...
  try {
    ...
    }, 12000); // 12s hard cap so we never hit 30s Heroku router timeout
...
// ---- UPDATED: bounded retries to avoid long loops ----
async function linkTokenCreateSmart(baseReq, prods) {
  ...
  while (products.length && attempts < 4) {
    ...
    try {
      ...
      throw e; // not a product list issue — surface quickly
    }
  }
