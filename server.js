// server.js — ACTIV one-file Plaid backend (no npm deps). Node 18+ (built-in fetch).
...
// Allowed/Preferred products (balances is NOT a product)
...
// Simple in-memory store (userId -> access_token)
...
function setCors(req, res){
  ...
  // Mirror whatever headers the browser asked for so preflight always passes.
...
const parseInvalidProducts = e => ((e?.error_message||'').match(/\[([^\]]+)\]/)||[])[1]?.split(',').map(s=>s.trim().toLowerCase())||[];
...
const server = http.createServer(async (req,res)=>{
  ...
  try{
    ...
    // Exchange public token (your UI may omit user id -> default)
    ...
    // Unlink (your UI calls /plaid/unlink)
    ...
    // Delete account (your UI calls /user/delete)
    ...
    // (Optional) other endpoints you might hit later
...
server.listen(PORT, ()=>console.log(`ACTIV backend running on :${PORT} (PLAID: ${PLAID_ENV})`));